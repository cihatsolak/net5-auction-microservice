namespace ESourcing.UI.Core.Common
{
    public class ResultConstants
    {
        public static string RecordFound = "Record found.";
        public static string RecordNotFound = "Record not found";
        public static string CreateSuccessfully = "Create successfully";
        public static string CreateNotSuccessfully = "Create not successfully";
        public static string AuctionCompleted = "Auction completed!";
        public static string AuctionNotCompleted = "Auction not completed!";
    }
}
